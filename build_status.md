...
21. **B8 promotion queue location + entry shape contract.** Bible 00 §12 line 387 names `promotion_queue.json` but doesn't canonize its location (T8 uses `paths.PROMOTION_QUEUE` per Phase 1) or the entry shape. ENTRY SHAPE CLOSED IN PHASE 3 T2 at `2e827f9`: `schemas/promotion_queue.py` defines `PromotionQueueEntry` (9 fields) + `PromotionQueue` wrapper. LOCATION CONTRACT STILL OPEN: bible should canonize `paths.PROMOTION_QUEUE` per the existing implementation. Defer remaining location contract to: future bible-edit pass.
...
23. **Bible 20 §5.2 line 148 `cee verify --boot` flag canonization.** Bible names the command verbatim. T9 implements as a flag on the existing `verify` subcommand (matches `--layout` / `--schemas` precedent from Phase 1). Closed at `3fdacb5` (T9).
24. **`_BOOT_HALT_HINTS` remediation content belongs in bible 19 §5.6.** Bible 19 §5.6 names "the user-facing message format" with "To resume" exact CLI commands as canonical content. T9 ships the boot-halt remediation hint table inline as a Python constant in `cli/commands/verify.py` (13 entries keyed by `(error_class_name, kind)`). Long-term, the canonical content should live in bible 19 §5.6, with `verify.py` reading from a structured bible-grounded source. Surface area: medium (one new bible section + a loader). Defer to: post-T9 bible-edit pass + Phase 5+ refactor. Superseded by candidate #58.
25. **Bible 20 §5.2 line 148 `cee verify --bible` flag canonization.** Bible names the command verbatim. T10 implements as a flag on the existing `verify` subcommand (matches `--layout` / `--schemas` / `--boot` precedent). Closed at `c01ea5b` (T10).
...
30. **Phase 3 monolithic vs split-phase decision record.** Bible 20 §5.3 bundles persistence (3 writers + audit) + safety gate (3 stages) + 3 CLI verbs into a single phase. AB resolved Q6 as monolithic — ship all of bible 20 §5.3 in one phase rather than splitting into Phase 3a (persistence) + Phase 3b (safety gate). Rationale: §5.3 gate criterion (d) `tests/integration/test_persistence_chain.py` couples both halves end-to-end; splitting would force a redundant intermediate gate. Resolution: no bible amendment needed; recorded here as the Phase 3 planning decision for future-AB reference. Closed at `5c58205` (Phase 3 T1).
31. **Bible canonization of `PromotionQueueEntry` field shape.** T2 defined the canonical schema (9 fields, two closed Literal types `PromotionKind` + `PromotionStatus`) because no bible section provides field-by-field shape. Bible references in 03/04/07 are at lifecycle-behavior level (status transitions, drain semantics, queue size limits) but don't enumerate fields. T2's four bible-grounding drift detectors guard the behaviors the schema rests on. Resolution: amend bible (likely bible 04 §X or bible 07 §X) to explicitly canonize the field set + Literal enums. Defer to: future bible-edit pass.
32. **Redactor hook in `persistence/filesystem_writer.py` deferred until T6.** [Original premise:] Bible 12 §11 line 470 says "filesystem_writer.py, obsidian_writer.py, notion_writer.py each call the redactor before writing." T3 shipped filesystem_writer.py before T6's redactor; a `# TODO #32 / bible 12 §5: invoke redactor here once T6 ships` marker was placed at the role-validation boundary. **CLOSED AT `4893895` (post-T6 reconciliation) AS BIBLE-MISREAD.** Bible 12 §5.7 canonical Detailed Workflow explicitly excludes filesystem_writer from substrate-redaction duty (only OBSIDIAN_WRITER and NOTION_WRITER re-run the redactor as defense-in-depth on derived substrates). Wire-up would corrupt audit-log hash chains, bible mirror canonical content, registry index.json files, and other structured filesystem state via false-positive regex matches. Bible §11 line 470 contradicts §5.7; §5.7 is the operationally-detailed canon. Resolution: TODO marker removed; replaced with §5.7-grounded explanatory comment. Marker-presence test replaced with negative-contract test asserting filesystem_writer does not import safety_gate. Bible §11 vs §5.7 contradiction surfaced as new downstream candidate #42.
33. **Bible 02 §7.8 SAFETY_GATE write paths not enumerated.** Bible names SAFETY_GATE writes as "redacted FinalPrompt + safety_log" but doesn't enumerate filesystem paths. T3's `_ALLOWED_WRITES` map infers `paths.RUNS_DIR` (FinalPrompt lives per-Run) + `paths.AUDIT_SECURITY_LOG` (safety log = security audit). Resolution: bible amend — add explicit "Writes to:" lines under §7.8 enumerating these paths (or correct T3's inference if the bible canonical disagrees). Defer to: future bible-edit pass.
34. **Bible 02 §7.13 BOOT_SEQUENCER `index.json` paths not enumerated.** Bible names BOOT_SEQUENCER's "rebuilt registries" output but bible 04 §6.5 separately names the `index.json` artifact paths; the cross-reference between §7.13 and §6.5 is implicit. T3 infers `paths.SKILLS_DIR / "index.json"` + `paths.AGENTS_DIR / "index.json"`. Resolution: bible amend — add explicit cross-reference from §7.13 to §6.5 (or enumerate the paths in §7.13). Defer to: future bible-edit pass.
35. **Bible 13 §11 named-public-API contract deferred to Phase 5+ renderer layer.** Bible 13 §11 specifies a per-artifact-type public API (`write_run(run_id)`, `write_skill(slug)`, `write_agent(slug)`, `write_bible_section(section)`) that wraps the underlying plumbing. T4 ships the plumbing layer (`write_artifact(kind, id, content)`); Phase 5+ renderers will compose the typed wrappers. TODO marker placed at module level: `# TODO #35 / bible 13 §11: Phase 5+ public API write_run/write_skill/write_agent/write_bible_section will wrap write_artifact() once renderers ship`. Surface area: small (4 thin wrappers on top of existing write_artifact). Closed by: Phase 5+ renderer wrap-up.
36. **Bible 13 §5.10 strict hash-and-skip idempotency deferred to Phase 5+ renderer layer.** Bible 13 §5.10 specifies a content-aware idempotency optimization (read existing → SHA → compare → skip on match) that lives at the renderer layer rather than the plumbing layer. T4 ships filesystem-level idempotency only — atomic overwrite of identical bytes is byte-identical via `atomic.atomic_write_text`. The content-aware skip is co-deferred with Phase 1's "Obsidian renderers" carry-forward. TODO marker placed in `write_artifact` docstring: `# TODO #36 / bible 13 §5.10: hash-and-skip idempotency optimization deferred to Phase 5+ renderer layer; T4 ships filesystem-level atomic overwrite only`. Closed by: Phase 5+ renderer wrap-up (alongside #35).
37. **Bible 12 §5.2 + §6.1 module location drift.** Bible 12 §5.2 + §6.1 reference `~/cee/security/redaction_patterns.py` as the catalog location. Bible 04 §5.1 + bible 20 §5.3 + bible 18 + Phase 3 plan canonize `safety_gate/redactor.py`. T6 ships at the latter (more recent + appears in build plan). Resolution: bible amend — update bible 12 §5.2 + §6.1 references to `safety_gate/redactor.py`. Defer to: future bible-edit pass.
38. **Bible 12 §5.2 street_address_us pattern markdown corruption.** The §5.2 catalog table cell for street_address_us is corrupted (broken regex split across cells, partial `\bd+s+[A-Z][a-z]+s+(St…Ave`). Pattern unimplementable as-shipped. T6 deferred this pattern entirely. Resolution: repair the markdown table cell with a clean regex, or excise street_address_us from the catalog if not actually canon. Defer to: future bible-edit pass.
39. **Bible 12 §5.2 silent on regex specifics for 3 patterns.** email, credit_card, ip_address are listed by name with prose-only descriptions ("standard RFC pattern", "Luhn-validated 13–19 digit groups", "IPv4 / IPv6") rather than concrete regex specifications. T6 deferred these 3 patterns rather than ship T6-defined regex. Each represents a real design call (RFC interpretation for email; algorithmic Luhn check for credit cards; IPv4-only vs IPv6 vs both for IP). Resolution: bible amend — canonize concrete regex for each. T6 v2 then implements with grounded source. Defer to: future bible-edit pass.
40. **Bible 12 §5.1 vs §7.2 drift on `term` field.** §5.1 pseudocode (line 105) includes `term=term` for user_term RedactionLogEntry instances; §7.2 JSON example shows only the three core fields (pattern, location, replaced_with). T6 ships `term: str | None = None` on RedactionLogEntry to handle both shapes — None-default serializes without the field (matching §7.2); explicit value serializes with it (matching §5.1). Resolution: bible amend — pick one canonical shape and align both sections. Defer to: future bible-edit pass.
...
45. **`InjectionScanResult` Pydantic wrapper for halt-envelope serialization.** Bible 12 §11 line 348 names `security.log` as audit destination but doesn't specify a per-Run `injection_scan_result.json` artifact. The §5.5 halt artifact is the matches list itself, surfaced via the Run's halt envelope. T7 ships matches as `list[InjectionMatch]` (frozen dataclass, not Pydantic). Pydantic wrapper for halt-envelope serialization deferred. Resolution: ship with the pipeline-integration task that defines the halt-envelope schema (likely Phase 4+ / Track C). Closed by: halt-envelope schema commit.

46. **Confirmation schemas (`Confirmation`, `ConfirmationRequest`) ship with T8.** [Original premise:] Bible 12 §7.3 names confirmation.json (3 fields: timestamp, command_used, operator_identity) and confirmation_request.json (bible silent on shape). Producer is safety_gate/confirmation.py per bible 12 §11 line 468. T7 deferred per Path (C); T8 co-locates schema with producer. **CLOSED AT `c37ed04` (Phase 3 T8).** Both schemas shipped in `schemas/confirmation.py` (one file, two coupled Pydantic models matching T6 RedactionLog pattern). SCHEMA_MANIFEST 18 → 20; both join "safety" category. ConfirmationRequest shape was bible-silent; T8 defined minimal viable, surface separate as #47.

47. **Bible 12 §7.3 silent on `confirmation_request.json` JSON shape.** Bible names the artifact path but doesn't enumerate fields. T8 defined minimal viable shape (run_id, action_description, affects, requested_at, confirm_command, cancel_command) derived from bible §5.4 lines 220-228 user-facing message template — every field surfaces somewhere in that message. Resolution: bible amend — canonize the ConfirmationRequest field set in §7.3 alongside the confirmation.json 3-field spec. Defer to: future bible-edit pass.

48. **`cee confirm <run_id>` and `cee abort <run_id>` CLI commands deferred to Track C.** Bible 12 §11 line 468 names these CLI commands as T8's module surface. T8 ships pure builders (`build_confirmation_request`, `build_operator_message`, `record_confirmation`) that the CLI consumes; CLI-side wiring (read halt.json, prompt user, call record_confirmation, write confirmation.json, unblock pipeline) defers to Track C (T9-T12). Surface area: medium (two CLI subcommands + run-state lifecycle integration). Closed by: Track C CLI commit.

49. **24h auto-abort background thread for destructive-confirmation timeout deferred.** Bible 12 §5.4 line 232 + §11 line 468 specify "After 24 hours without action, the Run is auto-aborted with reason confirmation_timeout" + "Background thread polls for timeout." T8 cannot ship this — it's persistent daemon or cron-style infrastructure, not a pure builder. Resolution: design + ship operational/CLI-infrastructure task (likely Phase 5+ ops surface, or a `cee daemon` command). Closed by: dedicated infrastructure commit.

50. **`produced_by` role on Confirmation schema: T8 chose RoleEnum.OPERATOR; bible canonical not pinned.** Bible 12 §7.3 line 384 wording ("written when OPERATOR confirms") makes OPERATOR the authoring identity; SAFETY_GATE merely persists. T8 chose `produced_by=RoleEnum.OPERATOR` per the actor-identity reading. RedactionLog uses `produced_by=RoleEnum.SAFETY_GATE` because SAFETY_GATE is the actor of redaction. The asymmetry is intentional: different artifacts, different actors. Resolution: bible amend — canonize `produced_by` for Confirmation (and confirm RedactionLog's choice) in bible §7.x. T8 documents the rationale inline in `schemas/confirmation.py` module docstring. Defer to: future bible-edit pass.

51. **Bible 07 §11 line 411 vs bible 20 §5.3 line 176 — module location drift for promotion queue mechanics.** Bible 07 §11 line 411 names `~/cee/skill_engine/promotion.py` for the queue API; bible 20 §5.3 line 176 names `~/cee/persistence/notion_writer.py`. Both bible sections name the same 4-function API surface (queue/drain/mark_approved/mark_rejected). T5 ships at the bible-20 location per Phase 3 plan + recency bias. Resolution: bible amend — pick one canonical location and update the other. Defer to: future bible-edit pass.

52. **Notion MCP write Protocol methods deferred until concrete transport ships.** T6's NotionMCPClient Protocol has read-only methods (connect, fetch_page_metadata, enumerate_children, fetch_page_blocks); current stub raises NotImplementedError on connect(). T5's drain() catches the stub-raise in the connect() branch. The post-connect per-entry write iteration is shipped as scaffold with `# TODO #52 / bible 04 §7.3: client.create_promotion_page(...) lands with concrete Notion MCP transport` marker. Mock-testable now via the test suite's mock client; activates without code restructure when concrete transport lands. Closed by: concrete Notion MCP transport commit.

53. **Notion-page-move detection (bible 07 §5.5 step 6).** Bible 07 §5.5 step 6 specifies "CEE detects the move on next sync, updates promotion_queue.json, clears needs_review on approval." T5 does not ship this — separate read pass over Notion that updates queue based on observed page locations is a distinct concern from drain(). Resolution: defer to post-concrete-transport task or boot B8 extension. Closed by: a `cee sync-promotions` CLI command or B8 extension commit.

54. **Bible silent on enqueue dedupe semantics, max_attempts policy, and idempotency on already-final lifecycle states.** T5 picks defaults documented in module docstring: dedupe by (slug, kind) with skip-silent semantics + audit event; no auto-rejection (attempts counter informational only per bible 01 §10.8); idempotent on already-final states (mark_approved on approved is no-op; mark_rejected on rejected is no-op). Resolution: bible amend — canonize each behavior in bible 07 §5.5 alongside the existing 6-step lifecycle. Defer to: future bible-edit pass.

55. **Vocabulary drift between bible 13 §5 line 186 (Obsidian frontmatter) and T2 PromotionStatus enum.** Bible 13 §5 line 186 specifies `promotion_status: pending | approved | rejected | not_queued` for Obsidian frontmatter; T2 PromotionStatus enum is `queued | pending_review | approved | rejected`. Different domains (Obsidian frontmatter vs. queue file) so not strictly contradictory, but mismatched terminology creates cognitive load. Resolution: bible amend — align vocabulary OR explicitly document the domain-specific divergence. Defer to: future bible-edit pass.

56. **Queue size cap (500) enforcement deferred to Track C `cee promote --flush`.** Bible 01 §10.8 specifies queue bounded at 500; warn at 50+; manual `cee promote --flush` clears. T5 ships warn-at-50 (`notion_queue_size_warn` audit event) but does NOT enforce 500-cap. Persistence-layer concern is signaling; CLI-layer concern is enforcement. Resolution: Track C `cee promote --flush` command implements pre-emptive drain when cap approached. Closed by: Track C cee promote --flush commit.

57. **Phase 2 boot sequencer `drain_queue(queue_path) -> dict` speculative API replaced by T5's bible-grounded `drain() -> DrainResult`.** Phase 2 T8 (sequencer) shipped B8 wiring against an unimplemented `drain_queue(queue_path)` API with `dict[drained, remaining]` return; tests monkeypatched `sys.modules["persistence.notion_writer"]` with a fake module exposing the speculative shape. T5 ships bible 07 §11 line 411's canonical `drain(*, client_factory=None) -> DrainResult` API; signature + return-type contract differs. **CLOSED AT `3c9ad81` (T5 Path A2).** Resolution: boot sequencer `_run_b8` migrated from speculative API to T5's bible-grounded API. B8 audit/halt payload migrated from 2-key shape (`drained`, `remaining`) to richer DrainResult-derived shape (`ok`, `attempted`, `succeeded`, `failed`, `skipped`, `transport_unavailable`) preserving full result taxonomy per bible 00 §12 B8 "Failures stay queued" contract. 4 tests migrated (3 unit + 1 integration drift-discovered). _audit_emit_b8_warning reasons updated: writer_pending + drain_failed retired; transport_unavailable + partial_failure + drain_raised added.

58. **Bible 19 §5.6 needs structured hint-table sub-section to receive Phase 3's five inline `_*_HINTS` constants.** Bible 19 §5.6 currently exists as a 2-line prose section ("Every halt and error produces a structured message…Generated by `errors/messages.py` from a template per halt type"). Phase 3 shipped 5 inline Python hint tables marked TODO #44 for back-port: `_BOOT_HALT_HINTS` (T9, `cli/commands/verify.py` — 13 entries), `_BIBLE_DRIFT_HINTS` (T10, `cli/commands/verify.py` — 4 entries), `_OBSIDIAN_VERIFY_HINTS` (T9, `cli/commands/verify.py` — 2 entries), `_AUDIT_VERIFY_HINTS` (T10, `cli/commands/audit_verify.py` — 3 entries), `_SCAFFOLD_OBSIDIAN_HINTS` (T11, `cli/commands/scaffold_obsidian.py` — 1 entry). Long-term, the canonical content should live in bible 19 §5.6 (expanded into a structured per-halt-class hint catalog), with the CLI modules reading from a structured bible-grounded source. Surface area: medium (one new bible sub-section + a loader). Defer to: future bible-edit pass + Phase 5+ refactor pass. Subsumes earlier candidates #24 + #27 which named the same back-port target individually.
...
**Status:** shipped 2026-05-03. Gate-passed at commit `a5a4673`. Test count: 1531 passing (Phase 2 close: 1134 → Phase 3 close: 1531; delta +397 across T1–T13).
...
**Reads:** `bible/20_production_build_plan.md` §5.3 (CLI surface), `bible/13_obsidian_integration.md` §5 (canonical layout), `cli/commands/verify.py` (Phase 2 pattern — `--layout` / `--schemas` / `--boot` / `--bible`), `persistence/obsidian_writer.py` (Task 4).
...
**Writes:** `cli/commands/audit_verify.py`, `cli/main.py` (subcommand registration), `tests/unit/test_cli/test_audit_verify_command.py`, `tests/unit/test_cli/test_main.py` (extended).
...
- [ ] Tests (`test_audit_verify_command.py`): all-clean → exit 0, planted broken chain → exit 1 with broken-line report, no audit logs (fresh state) → exit 0 with "no logs found" note.
- [ ] Commit: "Phase 3 task 10: cee audit-verify — hash-chain verification."

**Verification:** `pytest tests/unit/test_cli/test_audit_verify_command.py tests/unit/test_cli/test_main.py` passes; `cee audit-verify --help` shows the subcommand.
...
**Reads:** `bible/20_production_build_plan.md` §5.3 (CLI surface), `bible/13_obsidian_integration.md` §5 (canonical layout), `cli/commands/init.py` (Phase 1 pattern reference for directory-creation idempotency).
**Writes:** `cli/commands/scaffold_obsidian.py`, `cli/main.py` (subcommand registration), `tests/unit/test_cli/test_scaffold_obsidian_command.py`, `tests/unit/test_cli/test_main.py` (extended).
**Bible cross-refs:** bible 20 §5.3, bible 13 §5.
**Checklist:**
...
**Verification:** `pytest tests/unit/test_cli/test_scaffold_obsidian_command.py tests/unit/test_cli/test_main.py` passes; `cee scaffold-obsidian` creates the vault tree against fresh state.
...
**Effort:** L
**Status:** shipped at commit `1f0c3a8`. 9 tests, 530 lines.
**Goal:** Build the integration-level fixture proving a single test artifact round-trips through all three substrates per bible 20 §5.3 gate criterion #1. Uses real `persistence/atomic.py`, `persistence/audit.py`, `persistence/filesystem_writer.py` (T3), `persistence/obsidian_writer.py` (T4), `persistence/notion_writer.py` (T5 — enqueue + drain via `_ChainStubClient` mock).
**Reads:** `bible/20_production_build_plan.md` §5.3 gate criterion #1, `bible/04_database_file_structure.md` §10 (per-artifact write contract), `tests/integration/test_phase2_gate.py` (Phase 2 T11 — pattern reference for integration-fixture style), `persistence/*.py` (Tasks 3–5).
**Writes:** `tests/integration/test_persistence_chain.py` (530 lines, 9 tests).
**Bible cross-refs:** bible 20 §5.3 gate #1, bible 04 §10, bible 07 §11 (drain API).
**Shipped (5 corrections vs original spec):**
- Test artifact: Skill (the only `PromotionKind` literal that traverses all three substrates), not RawInput. Skill chosen because `PromotionKind = Literal["skill", "agent"]` per T2 schema.
- `chain_env` fixture invokes `cmd_scaffold_obsidian` (T11) directly with stdout suppressed via `contextlib.redirect_stdout`, not raw vault path-creation.
- Notion drain mocked via inline `_ChainStubClient` dataclass exposing `connect()` no-op + `create_promotion_page` recording method (TODO #52 injection point). Real Notion transport stays bible-04 §5.6 deferred.
- `obsidian_writer.write_artifact` invoked with pre-rendered note content (Phase 5+ renderers stay deferred); not the Phase 1 dispatch-shell `NotImplementedError`.
- Audit chain assertions: `notion_queued` + `notion_drain_attempted` + `notion_drain_succeeded` events recorded, plus full hash-chain integrity check via `verify_audit_chain` and a planted-tamper round-trip test.

**Verification:** `pytest tests/integration/test_persistence_chain.py` passes (9/9, ~0.12s).
...
**Effort:** M
**Status:** shipped across two commits — gate verification at `a5a4673` (commit 1 of 2), close-out paperwork at this commit (commit 2 of 2).
**Goal:** Confirm Phase 3 is complete per bible 20 §5.3's six gate criteria (lines 186–191 of bible 20). Mirrors Phase 2 Task 11 pattern.
**Reads:** `bible/20_production_build_plan.md` §5.3 (six gate bullets — load-bearing), `build_status.md` (this file), all Tasks 1–12 outputs, `tests/integration/test_phase2_gate.py` (Phase 2 T11 — pattern reference).
**Writes:**
- Commit 1 (`a5a4673`): `tests/integration/test_phase3_gate.py` (431 lines, 6 tests).
- Commit 2 (this commit): `build_status.md` (Phase 3 closed marker, 8 typo corrections, T12 spec amendment, T13 spec amendment, candidate #58 surfaced, candidate #59 surfaced, 57→59 sweep table, 8 in-line TBD-placeholder references replaced with actual resolution-commit hashes, supersession notes for #24 and #27).
**Bible cross-refs:** bible 20 §5.3 (six gate criteria), bible 12 §5.8 + §10.6 (audit chain shape exercised by criterion 4), `tests/integration/test_phase2_gate.py` (Phase 2 pattern).

**Six tests shipped (one per bible 20 §5.3 bullet):**
- `test_phase3_gate_criterion_1_round_trip_artifact_through_three_substrates` — bullet 1: subprocess-runs T12.
- `test_phase3_gate_criterion_2_redaction_patterns_all_covered` — bullet 2: introspects `_BUILTIN_PATTERNS` (10 shipped) against `test_redaction_patterns.py`.
- `test_phase3_gate_criterion_3_injection_patterns_all_covered` — bullet 3: introspects `_DIRECT_PATTERNS` (6) + `_HIDDEN_UNICODE_CHARS` mechanism against `test_injection_patterns.py`.
- `test_phase3_gate_criterion_4_hash_chain_detects_tampering` — bullet 4: minimal direct-tamper invariant via `audit_log_append` + `verify_audit_chain`.
- `test_phase3_gate_criterion_5_unit_test_suites_pass` — bullet 5: subprocess-runs `tests/unit/test_safety_gate/` + `tests/unit/test_persistence/` (313 tests combined).
- `test_phase3_gate_criterion_6_persistence_chain_integration_test_passes` — bullet 6: subprocess-runs T12 (textually nested with bullet 1; both shipped per bible-verbatim adherence).

**Phase-close precedent:** the rich closed-marker format (status line with shipped date, gate-passed commit hash, test count delta) inserted under the Phase 3 section header by this commit is the precedent for future phase close-outs. Phase 4+ should mirror this format.

**Verification:** Phase 3 gate per bible 20 §5.3 is satisfied. `pytest tests/integration/test_phase3_gate.py` passes 6/6. Full test suite: 1531 passing.

---

### Phase 3 close — downstream-candidate sweep

Status of every downstream-reconciliation candidate at Phase 3 close.

| # | Status | Resolution-commit ref |
|---|--------|------------------------|
| #15 | Closed | `120cf57` (Phase 2 T8 — pyproject `requires-python` aligned) |
| #21 | Partially closed (entry shape closed; location contract still open) | `2e827f9` (Phase 3 T2 — `PromotionQueueEntry` schema) |
| #23 | Closed | `3fdacb5` (Phase 3 T9 — `cee verify --obsidian`) |
| #25 | Closed | `c01ea5b` (Phase 3 T10 — `cee audit-verify`) |
| #30 | Closed | `5c58205` (Phase 3 T1 — task plan ratified monolithic Phase 3 decision) |
| #32 | Closed (bible-misread; #42 surfaced) | `4893895` (post-T6 reconciliation) |
