...
46. **Confirmation schemas (`Confirmation`, `ConfirmationRequest`) ship with T8.** Original T7 prompt erroneously asked for `ConfirmationLog` schema as part of T7. Bible 12 §7.3 actually names two artifacts: `confirmation.json` (3 fields per bible: timestamp, command_used, operator_identity) and `confirmation_request.json` (bible silent on shape). Producer is `safety_gate/confirmation.py` per bible 12 §11 line 468 — i.e., T8, not T7. T7 ships only injection scanner per Path (C) resolution; confirmation schemas land with T8's confirmation handler. Co-located producer-and-schema follows the established pattern (T2 PromotionQueueEntry, T6 RedactionLog). T8 will close this candidate.
