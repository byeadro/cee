...
21. **B8 promotion queue location + entry shape contract.** Bible 00 §12 line 387 names `promotion_queue.json` but doesn't canonize its location (T8 uses `paths.PROMOTION_QUEUE` per Phase 1) or the entry shape. ENTRY SHAPE CLOSED IN PHASE 3 T2 (commit ref TBD): `schemas/promotion_queue.py` defines `PromotionQueueEntry` (9 fields) + `PromotionQueue` wrapper. LOCATION CONTRACT STILL OPEN: bible should canonize `paths.PROMOTION_QUEUE` per the existing implementation. Defer remaining location contract to: future bible-edit pass.
