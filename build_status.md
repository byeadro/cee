...
32. **Redactor hook in `persistence/filesystem_writer.py` deferred until T6.** Bible 12 §5 line 470 says "filesystem_writer.py, obsidian_writer.py, notion_writer.py each call the redactor before writing." T3 ships filesystem_writer.py before T6's redactor; a `# TODO #32 / bible 12 §5: invoke redactor here once T6 ships` marker is placed at the role-validation boundary in `_assert_role_can_write`'s caller path. Resolution: post-T6, filesystem_writer.py needs a redactor-call wire-up pass. Surface area: small (single hook insertion + redaction-skip-flag for redactor's own audit emissions). Closed by: post-T6 wire-up commit.
