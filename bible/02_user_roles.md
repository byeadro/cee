...
- **Authority:** sequences step invocations; catches `CEEException` from any step; emits lifecycle artifacts (summary, halt, error). Does NOT produce step artifacts (those carry their step's own `produced_by`). Cannot modify upstream artifacts. Cannot bypass `PERSISTENCE_WRITER` for canonical step artifact writes — only owns Run-lifecycle artifacts.
- **Audit:** every step start, every step success, every halt, every error, every artifact write logged to the Run's `pipeline.log`. Lifecycle artifacts carry `produced_by: "PIPELINE_DRIVER"`.
...
| Role | Produces |
| --- | --- |
| `INTERPRETER` | `IntentObject` |
| `CLASSIFIER` | `Classification` |
| `AGENT_SELECTOR` | `AgentPlan` |
| `SKILL_ENGINE` | `SkillSet` (+ optional [SKILL.md](http://SKILL.md) files) |
| `STRATEGY_BUILDER` | `ExecutionStrategy` |
| `PROMPT_BUILDER` | `FinalPrompt` |
| `SAFETY_GATE` | safety-annotated `FinalPrompt` |
| `PERSISTENCE_WRITER` | filesystem state changes |
